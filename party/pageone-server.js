/* ページワン オンライン対戦サーバー。
   全手札と山札はDO内だけに保持し、private-state.js で本人手札＋公開情報へ投影する。
   3〜4人、空席CPU補充、seat token再接続、切断60秒後CPU代打ち。 */
import {
  normalizePageOnePlayers, makePageOneDeck, shufflePageOne, newPageOneRound,
  pageOneTop, pageOnePlay, pageOneDraw, pageOneCpuChoose,
...
export default class PageOneServer {
  constructor(room) {
    this.room = room;
    this.game = null;
  }
  ...
  async onStart() {
    ...
    g.rules = { playerCount: normalizePageOnePlayers(g.rules && g.rules.playerCount) };
  ...
  emitResult(result, seat) {
    const g = this.game;
    if (result.pageOneMiss) this.toastAll(g.seats[seat].name + 'さん ページワン わすれ！ +2');
    if (result.finishedSeat != null) this.toastAll(g.seats[seat].name + 'さん あがり！');
    if (result.ended) g.phase = 'ended';
  }

  runCpuChain() {
    const g = this.game;
    let guard = 0;
    while (g.phase === 'playing' && g.round && !g.round.ended && guard++ < 1000) {
      const seat = g.round.turn, s = g.seats[seat];
      if (!s || !s.cpu) break;
      const move = pageOneCpuChoose(g.round, seat);
      const result = move.type === 'play'
        ? pageOnePlay(g.round, seat, move.cardId, { suit: move.suit, pageOne: true }, () => this.rand())
        : pageOneDraw(g.round, seat, () => this.rand());
      if (!result.ok) break;
      this.emitResult(result, seat);
    }
  }
  ...
  startRound() {
    const g = this.game;
    g.gameNo++;
    g.round = newPageOneRound(g.rules.playerCount, shufflePageOne(makePageOneDeck(), () => this.rand()));
  ...
  async onMessage(raw, conn) {
    ...
    if (m.type === 'setPlayers') {
      ...
      const playerCount = normalizePageOnePlayers(m.playerCount);
    ...
    if (m.type === 'play' || m.type === 'draw') {
      if (g.phase !== 'playing' || !g.round) return this.err(conn, 'たいきょくちゅうでは ありません');
      if (seat < 0) return this.err(conn, 'かんせんちゅうは うてません');
      const result = m.type === 'play'
        ? pageOnePlay(g.round, seat, m.cardId, { suit: m.suit, pageOne: m.pageOne === true }, () => this.rand())
        : pageOneDraw(g.round, seat, () => this.rand());
      if (!result.ok) return this.err(conn, result.error || 'その ては できません');
      this.emitResult(result, seat);
      this.runCpuChain();
      await this.save();
      this.broadcastState();
  ...
  stateFor(conn) {
    ...
    if (r) Object.assign(publicState, {
      top: pageOneTop(r),
      activeSuit: r.activeSuit,
      turn: r.turn,
      direction: r.direction,
      pendingDraw: r.pendingDraw,
      pendingKind: r.pendingKind,
