...
### 3b. 五目並べ オンライン対戦（実装済み・URLを入れるだけ）
バックエンドは同リポジトリの [`party/`](party/)（PartyKit = Cloudflare Durable Objects、部屋=1 Party）。
サーバーが盤面を権威的に検証し、部屋コード/リンク共有で2人対戦＋再接続＋観戦ができる。
デプロイ手順は [party/README.md](party/README.md)。デプロイ後 `PGP_CONFIG.partyHost` にホスト名を入れるだけ。
**未設定なら五目並べの「🌐 オンライン対戦」は『準備中』で無効化され、ローカル対戦・他ゲームは無傷**。
